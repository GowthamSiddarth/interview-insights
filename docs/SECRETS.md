...
**Pattern A, hybrid root (D78, extended by GitHub issue #803/Phase 55):**
`admin-credentials`/`anthropic-credentials`/`email-secrets` are Pattern B
*and* feed Pattern A — provisioned imperatively exactly like the pure-B
secrets, but also wired into LocalStack's own pod so its init-hook can
seed Secrets Manager with the real value on every start. Unlike
`admin-credentials` (no dev-only placeholder possible for a bcrypt hash),
`email-secrets`'s three values (`EMAIL_HASH_SECRET`/
`EMAIL_ENCRYPTION_KEY`/`CANDIDATE_JWT_SECRET`) are plain random strings —
`bootstrap-kind.sh`/`cd.yml` generate them, so no human ever has to pick
or supply one; see wiki/deployment-guide.md 5e. `api` only ever talks to
Secrets Manager for `admin-credentials`/`email-secrets`; `anthropic-api-key`
moved to `review-analyzer` as of GitHub issue #340 (D81) — see the
dedicated section below for why the
...
| `interview-insights/email-hash-secret` | A, hybrid root (GitHub issue #803) | `api` only | `EMAIL_HASH_SECRET` |
| `interview-insights/email-encryption-key` | A, hybrid root (GitHub issue #803) | `api`, `notification-service` | `EMAIL_ENCRYPTION_KEY` (D74: byte-identical across both — same Secrets Manager entry, by construction) |
| `interview-insights/candidate-jwt-secret` | A, hybrid root (GitHub issue #803) | `api` only | `CANDIDATE_JWT_SECRET` |
...
| `email-secrets` (k8s Secret) | B, root for the hybrid case above (GitHub issue #803) | LocalStack's own pod — never `api`/`notification-service` directly | `EMAIL_HASH_SECRET`, `EMAIL_ENCRYPTION_KEY`, `CANDIDATE_JWT_SECRET` — generated by `bootstrap-kind.sh`/`cd.yml`, never a human-picked value |
...
- `infra/aws/seed-localstack.sh` — runs *outside* the container (CD,
  ...
  IAM role/policy pairs. `SEED_DATABASE_URL` is the one true
  default-with-override: `DATABASE_URL`'s password comes from
  `$POSTGRES_PASSWORD` (D77) — keeping this in sync with the real
  Postgres password is why `cd.yml`'s "Seed LocalStack secrets + IAM"
  step explicitly passes it through — falling back to the literal
  `postgres` only for the no-real-Postgres LocalStack practice flow.
  Every other secret is **required, not defaulted** (GitHub issue #803,
  Phase 55, extended this to `SEED_EMAIL_HASH_SECRET`/
  `SEED_EMAIL_ENCRYPTION_KEY`/`SEED_CANDIDATE_JWT_SECRET`, previously the
  three secrets with a checked-in placeholder default — a real,
  functioning secret committed to the repo, exactly what hard constraint
  #6 forbids): `$ADMIN_PASSWORD_HASH`/`$ADMIN_JWT_SECRET`/
  `$SEED_EMAIL_HASH_SECRET`/`$SEED_EMAIL_ENCRYPTION_KEY`/
  `$SEED_CANDIDATE_JWT_SECRET` all hard-fail if unset (`$ANTHROPIC_API_KEY`
  is the one exception, genuinely optional — skips creating the secret
  entirely rather than seeding an empty one, see the gotcha below),
  matching `bootstrap-kind.sh`'s own required-env-var checks — except the
  three email/candidate ones, which `bootstrap-kind.sh`/`cd.yml` generate
  themselves rather than requiring a human to supply (wiki/deployment-
  guide.md 5e).
- `infra/k8s/base/localstack/init/seed.sh` — LocalStack's own
  ...
  for `database-url` (hardcoded `postgres` password fallback, since it
  runs inside the container with the bundled `awslocal` wrapper, not via
  the outside-facing AWS CLI the other script needs) — but for the six
  hybrid-root secrets (GitHub issue #803, Phase 55 added
  `email-hash-secret`/`email-encryption-key`/`candidate-jwt-secret` to
  the three D78 originally established), it reads them from **its own
  container environment** instead of a hardcoded value (see below for
  why).
...
a bcrypt hash isn't something a script can generate on its own the way
`email-secrets`'s three plain-random-string values now can (GitHub issue
#803, Phase 55 moved those onto this same hybrid pattern too, closing
the exact gap this section's own contrast used to point at). That
...
| `ghcr-pull-secret` | B (D105, new — #660) | k3s node, via each Deployment's `imagePullSecrets` | n/a — a `kubernetes.io/dockerconfigjson`, not a literal env var | `HETZNER_GHCR_PULL_PAT` — deliberately a *different*, `read:packages`-only token from `HETZNER_GHCR_PAT` (GitHub issue #809, Phase 55): the pull secret sits on the cluster where anyone who can read that k8s Secret gets whatever scope the token carries, so it must never carry `write:packages`. `HETZNER_GHCR_PAT` stays `write:packages`-scoped and is used only in GitHub Actions itself, for the `docker login ghcr.io` push steps — it never reaches the cluster. |

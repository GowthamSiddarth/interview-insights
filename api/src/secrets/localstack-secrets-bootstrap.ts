...
// GitHub issue #466's own follow-up (D78) added admin-credentials, which
// used to bypass this bootstrap entirely (wired directly into api's
// Deployment) — every secret api reads now comes from exactly this one
// path. anthropic-credentials moved to review-analyzer's own bootstrap as
// of GitHub issue #340 (D81) — api no longer calls the LLM at all.
//
...
export async function bootstrapSecretsFromLocalStack(): Promise<void> {
  ...
  const [databaseUrl, emailHashSecret, emailEncryptionKey, candidateJwtSecret, adminPasswordHash, adminJwtSecret] =
    await Promise.all([
